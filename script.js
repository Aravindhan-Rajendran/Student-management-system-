...
function display(student) {
  ...
  for (let i = 0; i < student.length; i++) {
      ...
      idCell.width = "103px";
      idCell.textContent = student[i].id;
      row.appendChild(idCell);
      let nameCell = document.createElement("td");
      let img=document.createElement("img");
      img.src=student[i].img_src;
      img.width="50";
      img.height="50";
      img.style.border = "1px solid gray";
      img.style.borderRadius = "50%";
      nameCell.appendChild(img);
      let name = document.createTextNode(student[i].first_name + " " + student[i].last_name);
      nameCell.appendChild(name);
      row.appendChild(nameCell);
      nameCell.width = "600px";
      let genderCell = document.createElement("td");
      genderCell.textContent = student[i].gender;
      row.appendChild(genderCell);
      genderCell.width = "150px";
      let classCell = document.createElement("td");
      classCell.textContent = student[i].class;
      row.appendChild(classCell);
      classCell.width = "250px";
      let markCell = document.createElement("td");
      markCell.textContent = student[i].marks;
      row.appendChild(markCell);
      markCell.width = "250px";
      let passCell = document.createElement("td");
      passCell.textContent = student[i].passing;
      row.appendChild(passCell);
      passCell.width = "250px";
      let emailCell = document.createElement("td");
      emailCell.textContent = student[i].email;
      row.appendChild(emailCell);
      emailCell.width = "150px";
...
function gend() {
  student.sort((a, b) => {
      let malee = a.gender.toUpperCase();
      let femalee = b.gender.toUpperCase();
      if (malee < femalee) {
          return -1;
      }
      if (malee > femalee) {
...
filteredStudents = student.filter(s => 
  s.first_name.toLowerCase().includes(searchValue) ||
  s.gender.toLowerCase().includes(searchValue) ||
  s.class.toString().includes(searchValue) ||
  s.marks.toString().includes(searchValue) ||
